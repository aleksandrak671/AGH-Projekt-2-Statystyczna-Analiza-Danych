...
```{r}
# Zapis danych używanych w projekcie

# 1) Pełny plik po czyszczeniu (ze wszystkimi kolumnami)
write.csv(countries_clean, "countries_clean.csv", row.names = FALSE)

# 2) Tylko zmienne użyte w PCA/MDS (standaryzowane)
countries_scaled_df <- as.data.frame(countries_pca_scaled)
colnames(countries_scaled_df) <- zmienne_num
write.csv(countries_scaled_df, "countries_clean_scaled.csv", row.names = FALSE)
```
...
Im mniejszy STRESS (bliżej 0), tym lepsze odwzorowanie:
 ...
 - większe od 0.20 – rzutowanie 2D mocno upraszcza odległości.
...
```{r}
# STRESS klasyczny (ten sam wzór co dla cmdscale, dla porównywalności)
d_orig   <- as.numeric(dist_countries)
d_sammon <- as.numeric(dist(mds_sammon_points[ , c("Dim1", "Dim2")]))

stress_sammon_classical <- sqrt( sum( (d_orig - d_sammon)^2 ) / sum(d_orig^2) )
stress_sammon_classical
```

Dla metody Sammona otrzymaliśmy STRESS ≈ 0.16. Jest to mniej niż dla klasycznego MDS w 2D (≈ 0,21).
...
porownanie_stress <- data.frame(
  ...
  STRESS = c(stress_cmd, stress_cmd3, stress_sammon_classical)
)
...
W tabeli widać, że największy błąd (STRESS) ma klasyczny MDS w 2D (≈ 0,21), mniejszy w 2D Sammona (≈ 0,16), a najmniejszy błąd – rzutowanie klasycznego MDS do 3D (≈ 0,13). Im mniejszy STRESS, tym lepsze odwzorowanie odległości między krajami. U nas wynik 3D jest najlepszy (najmniejszy STRESS).
...
- **cmdscale 2D** – STRESS ≈ 0,21: rzutowanie 2D dość mocno zniekształca odległości, trzeba uważać przy interpretacji.  
- **cmdscale 3D** – STRESS ≈ 0,13: reprezentacja 3D wyraźnie lepiej odwzorowuje odległości, lecz trudniej ją pokazać graficznie.  
- **Sammon 2D** – STRESS ≈ 0,16: jest lepszy od klasycznego MDS 2D, ale gorszy niż 3D.
...
- **Metoda Sammona**  
  + zalety: lepiej zachowuje małe odległości (lokalne podobieństwa) niż klasyczny MDS 2D,  
  + wady: metoda jest wolniejsza (iteracyjna), wynik zależy od punktu startowego i może zatrzymać się w rozwiązaniu tylko lokalnie najlepszym; STRESS także nie jest tak niski jak w 3D.

Ogólnie: układ grup krajów jest podobny w cmdscale i w Sammonie; rzutowanie do 3D daje najlepszą zgodność odległości, natomiast Sammon 2D daje poprawę względem klasycznego MDS 2D.
...
# Wektory własne: odpowiadają kolejnym wartościom własnym
...
W pca_prcomp mamy m.in.:
 ...
 - pca_prcomp$rotation – wektory własne,
...
Teraz patrzymy na wektory własne –> czyli jak każda zmienna "wchodzi" do danej składowej.
```{r}
# wektory własne z prcomp()
...
Korelacja PC1 z MDS1 ≈ –1. 00 oraz PC2 z MDS2 ≈ –0.93 potwierdza, że obie metody pokazują to samo ułożenie krajów, tylko osie są czasem odwrócone (co w PCA/MDS jest normalne i nie ma znaczenia).
...
- **MDS (Sammon)**
  ...
  - STRESS 2D ≈ 0.16 — czyli jakość odwzorowania jest średnia; lepsza niż w klasycznym MDS 2D, ale wyraźnie gorsza niż w 3D,
  - **osie nie mają prostego znaczenia** – ważne są tylko odległości.

- **PCA**
  - pokazuje **to samo ogólne ułożenie krajów**,
...
**Podsumowując:**  
Najlepsze odwzorowanie odległości daje rzutowanie MDS do 3D (cmdscale 3D, STRESS ≈ 0.13). Sammon 2D poprawia dopasowanie względem klasycznego MDS 2D, ale także nie oddaje wszystkich zależności idealnie (STRESS ≈ 0.16).
